...
declare
  ...
  v_slot_id uuid;
begin
  insert into sepak.sessions (session_no, title, play_date, start_time, venue, fee_myr)
  values (1, 'Test', '2026-09-16', '20:00:00', 'Padang Test', 27)
  returning id into v_session_id;

  -- defaults land as specified
  assert (select duration_mins from sepak.sessions where id = v_session_id) = 120,
  ...
  assert (select team_b_name from sepak.sessions where id = v_session_id) = 'Putih',
  ...
  begin
    update sepak.slots set player_name = 'Hazmi' where id = v_slot_id;
  ...
  begin
    insert into sepak.slots (session_id, team, position) values (v_session_id, 'A', 'GK');
  ...
  assert (select count(*) from sepak.slots where session_id = v_session_id and team = 'A') = 3,
    'CB1 and CB2 should both be storable';

  -- team and position are constrained to known values
  begin
    insert into sepak.slots (session_id, team, position) values (v_session_id, 'D', 'GK');
    raise exception 'expected the team check constraint to fire';
  exception
    when check_violation then null;
  end;
  begin
    insert into sepak.slots (session_id, team, position) values (v_session_id, 'A', 'SWEEPER');
  ...
  assert (select count(*) from sepak.slots where session_id = v_session_id) = 0,
