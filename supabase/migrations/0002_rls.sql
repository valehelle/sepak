alter table sepak.sessions enable row level security;
alter table sepak.slots    enable row level security;

-- Everyone may read: the booking list is public by design.
create policy sessions_read on sepak.sessions
  for select to anon, authenticated using (true);

create policy slots_read on sepak.slots
  for select to anon, authenticated using (true);
...
grant select, insert, update, delete on sepak.sessions, sepak.slots to authenticated;
...
revoke all on sepak.sessions from anon;
revoke all on sepak.slots    from anon;

grant select on sepak.sessions to anon;
...
-- through sepak.my_slot_ids(), which requires presenting the token, not
-- reading it off the row.
grant select (id, session_id, team, position, player_name, claimed_at)
  on sepak.slots to anon;
