...
comment on column sepak.waitlist.claim_token is
...
comment on column sepak.waitlist.positions is
...
create index waitlist_session_created_idx on sepak.waitlist (session_id, created_at);

-- Realtime pushes queue changes to every open session page, mirroring slots.
alter publication supabase_realtime add table sepak.waitlist;
...
alter table sepak.waitlist replica identity full;

---------------------------------------------------------------------------
-- RLS + grants
---------------------------------------------------------------------------
alter table sepak.waitlist enable row level security;

-- The queue is public, like the booking list itself.
create policy waitlist_read on sepak.waitlist
  for select to anon, authenticated using (true);

-- Same discipline as sessions_write/slots_write post-0006_admins.sql:
-- membership in sepak.admins, not merely being authenticated, is what
...
create policy waitlist_write on sepak.waitlist
...
revoke all on sepak.waitlist from anon;
revoke all on sepak.waitlist from authenticated;
...
grant select (id, session_id, player_name, positions, created_at)
  on sepak.waitlist to anon;

grant select, insert, update, delete on sepak.waitlist to authenticated;
...
grant select, insert, update, delete on sepak.waitlist to service_role;
...
-- leave_waitlist
---------------------------------------------------------------------------
create or replace function sepak.leave_waitlist(p_session_id uuid, p_token uuid)
returns void
language plpgsql
security definer
set search_path = sepak, pg_temp
...
---------------------------------------------------------------------------
create or replace function sepak.my_waitlist_entry(p_session_id uuid, p_token uuid)
...
revoke all on function sepak.join_waitlist(uuid, text, text[], uuid) from public;
revoke all on function sepak.leave_waitlist(uuid, uuid)              from public;
revoke all on function sepak.my_waitlist_entry(uuid, uuid)           from public;

grant execute on function sepak.join_waitlist(uuid, text, text[], uuid) to anon, authenticated;
...
create trigger slots_fill_from_waitlist
  after update on sepak.slots
  for each row
  when (OLD.player_name is not null and NEW.player_name is null)
  execute function sepak.fill_from_waitlist();
